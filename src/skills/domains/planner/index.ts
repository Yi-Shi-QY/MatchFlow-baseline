...
function collectBuiltinTemplates(): PlanTemplate[] {
  const modules = import.meta.glob("../*/planner/index.ts", { eager: true }) as Record<
    string,
...
function getDefaultTemplate(): PlanTemplate {
  ...
  if (!preferred) {
    throw new Error("No built-in plan templates discovered under skills/domains/*/planner.");
  }
  return preferred;
}
