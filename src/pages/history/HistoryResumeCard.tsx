...
import type { HistoryResumeCardModel } from './historyWorkspaceModel';

interface HistoryResumeCardProps {
  model: HistoryResumeCardModel;
  onOpen: (route: string, state: Record<string, unknown>) => void;
}

export function HistoryResumeCard({ model, onOpen }: HistoryResumeCardProps) {
  return (
    <article className="rounded-[1.5rem] border border-[var(--mf-border)] bg-[var(--mf-surface)]/92 p-4 shadow-sm">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--mf-text-muted)]">{model.reason}</p>
