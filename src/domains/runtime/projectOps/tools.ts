...
export function mapLegacyManagerEffectToProjectOpsRuntimeToolResult(effect: {
  agentText: string;
  messageKind: 'text' | 'draft_bundle';
  draftIds?: string[];
  action?: unknown;
...
export const projectOpsRuntimeTools: DomainToolDefinition[] = [
  {
    id: 'project_ops_explain_capability',
    description: 'Explain project ops factors, sequence, or general manager help.',
    canHandle(input) {
  ...
  {
    id: 'project_ops_prepare_task_intake',
    description: 'Prepare project ops task intake and start workflow when clarification is needed.',
    canHandle(input) {
  ...
  {
    id: 'project_ops_help',
    description: 'Fallback project ops guidance for ambiguous input.',
    canHandle() {
      return true;
    },
    async execute(input) {
